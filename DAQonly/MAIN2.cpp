...
int RTC::getTimeStamp() {
  ...
  //  else {
        ...
        RTCdata.startTime_min = timeStamp[5];
        timeStamp[4] = i2c.receive(registerAddress[4],readBuf,1) & 0x3F; // hour
        RTCdata.startTime_hour = timeStamp[4];
        ...
        RTCdata.startTime_day = timeStamp[2];
        timeStamp[1] = i2c.receive(registerAddress[1],readBuf,1) & 0x1F; // month
        RTCdata.startTime_month = timeStamp[1];
...
void IMU::setupGyro(){
    ...
    writeBuf[0] = GYRO_REGISTER_CTRL_REG4; //
    writeBuf[1] = 0x70; //0x70 // 0x30 //40 //00
    i2c.send(writeBuf, 2);
    ...
    for (int n=1; n<=100; n++ ) {
        xCal = getGyroValue(1);
        xCalSum = xCalSum + xCal;
        yCal = getGyroValue(2);
        yCalSum = yCalSum + yCal;
        zCal = getGyroValue(3);
        zCalSum = zCalSum + zCal;
    }
    float xCalGyroValue= xCalSum / 100 ;
    float yCalGyroValue= yCalSum / 100 ;
    float zCalGyroValue= zCalSum / 100 ;
...
float IMU::getGyroValue(int channel){
    ...
    switch (channel) {  // IMU Channel Selection
            ...
            // ---------------------- GYRO---------------------//
        case 1: // channel 1: x-roll
            value = value - xCalGyroValue;
            break;
        case 2: // channel 2: y-pitch
            value = value - yCalGyroValue;
            break;
        case 3: // channel 3: z-yaw
            value = value - zCalGyroValue;
            break;
    ...
    printf("%f roll rate",  value);
    
    printf("%f angle",  0.01*value);
...
int GPIO::isrInit(){
    ...
    if (wiringPiISR (ISR_PIN, INT_EDGE_RISING, &interrupt) < 0)	{ // edge triggered
...
//    else {
    ...
    fprintf(stream, "%f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f\n", ADCdata.steerAngle[i], ADCdata.airSpeed[i], ADCdata.brakePressB[i], ADCdata.fuelTankLevel[i], ADCdata.shockDispFL[i], ADCdata.shockDispFR[i], ADCdata.shockDispBR[i], ADCdata.shockDispBL[i], ADCdata.channel9[i], ADCdata.channel10[i], ADCdata.channel11[i], ADCdata.channel12[i], INAdata.brakePressF[i], IMUdata.xAccel[i], IMUdata.yAccel[i], IMUdata.zAccel[i], IMUdata.xRot[i], IMUdata.yRot[i], IMUdata.zRot[i], IMUdata.temp[i]);//, GPS.GPSRMC_sentence[i], GPS.speed[i]);
    fclose(stream);
    }
...
bool STORAGE::fexists(const char *filename)
{
...
float GPS::getValue (unsigned char channel){
