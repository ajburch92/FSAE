...
void Adafruit_INA219::setup() {
    ...
    ina219.setCalibration_16V_400mA();  // make calibration adjustments for the change of resistor
...
void interrupt () {
    ...
    //printf("%f    ",  IMUdata.xAccel[countOverload]);
    //printf("%f    ",  IMUdata.yAccel[countOverload]);
    //printf("%f \n",  IMUdata.zAccel[countOverload]);
    //INAdata.brakePressF[countOverload] = ina219.getValue();
    
    
    //GPS_sentence = gps.getGPSRMC();
    //GPSdata.GPSRMC_sentence[countOverload].push_back(GPS_sentence);
    //GPS_sentence.copy(GPSdata.GPSRMC_sentence[countOverload], sizeof GPSdata.GPSRMC_sentence[countOverload]#define ISR_PIN      6 //define GPIO6);
    //GPSdata.speed[countOverload] = gps.getSpeed(GPSdata.GPSRMC_sentence[countOverload]);
    
    printf("stored value: %f, %f ", ADCdata.shockDispFL[countOverload], ADCdata.shockDispFL[countOverload]);
...
int main(int argc, char **argv) {
    ...
    ADCdata.shockDispFL[countOverload] = adc.getValue(1);
    ADCdata.shockDispFR[countOverload] = adc.getValue(2);
    ADCdata.shockDispBR[countOverload] = adc.getValue(3);
    ADCdata.shockDispBL[countOverload] = adc.getValue(4);
    ADCdata.shockDispFL[countOverload] = adc.getValue(5);
    ADCdata.shockDispFR[countOverload] = adc.getValue(6);
    ADCdata.shockDispBR[countOverload] = adc.getValue(7);
    ADCdata.shockDispBL[countOverload] = adc.getValue(8);
    ADCdata.shockDispFL[countOverload] = adc.getValue(9);
    ADCdata.shockDispFR[countOverload] = adc.getValue(10);
    ADCdata.shockDispBR[countOverload] = adc.getValue(11);
    ADCdata.shockDispBL[countOverload] = adc.getValue(12);
...
int main(int argc, char **argv) {
	...
	Tuple imuvalue;
	while (1){
		//imuvalue.low = imu.getValue(1).low;
		//imuvalue.high = imu.getValue(1).high;
		printf("%d\n",imu.twosComplement(imu.getValue(3).low,imu.getValue(3).high));
	}
	return 0;
}
 */
